...
static bool linkTwoModules(llvm::Module *Dest,
  ...
  auto linkResult = Linker::linkModules(*Dest, std::move(Src), flags);
...
void klee::linkModules(llvm::Module *composite,
  ...
  for (auto &module : modules) {
    if (!module)
      continue;
    if (!linkTwoModules(composite, std::move(module), flags, errorMsg)) {
      // Linking failed
...
bool klee::loadFileAsOneModule(const std::string &fileName, LLVMContext &context,
                     std::vector<std::unique_ptr<llvm::Module>> &modules,
                     std::string &errorMsg) {
